...
def login_center():
    ...
    if username_enter.get() != b and entered_password != a:
        warn = "Seems you are new here.\nSign Up to continue"
    elif username_enter.get() == b and entered_password != a:
...
def reset():
    ...
    uname_info = uname_enter.get()
    email_info = email_enter.get()
    new_pas = new_pas_enter.get()

    if uname_info == '' and email_info=='':
        warn = 'Please enter your user name and registered email id'
    else:
        check += 1

    my_query = {'_id':uname_info}
    ...
    if check == 3:
        try:
            ...
            mycol.update_one(my_query,{'$set':{'Passcode':new_pas}})
            msg.showinfo('Info', 'Reset operation successful!')
            login_frame.pack(fill='both', expand=True)
            reset_frame.forget()

        except Exception as ep:
            msg.showerror('Error','Something went wrong. Trye again')
    else:
        msg.showerror('Error', warn)

    uname_enter.delete(0, END)
    email_enter.delete(0, END)
    new_pas_enter.delete(0, END)
...
pas_enter = Entry(login_frame, textvariable=password_var, bd=2, width=30, show='*', justify=CENTER)
...
snfn_entry = Entry(signin_frame, textvariable=sn_fname_var, bd=2, width=30, justify=CENTER)
snlsn_entry = Entry(signin_frame, textvariable=sn_lname_var, bd=2, width=30, justify=CENTER)
snmail_entry = Entry(signin_frame, textvariable=sn_email_var, bd=2, width=30, justify=CENTER)
snun_entry = Entry(signin_frame, textvariable=sn_username_var, bd=2, width=30, justify=CENTER)
snpas_entry = Entry(signin_frame, textvariable=sn_password_var, bd=2, width=30, show='*', justify=CENTER)
...
uname_enter = Entry(reset_frame, textvariable=user_name_var, bd=2, width=30, justify=CENTER)
uname_enter.place(x=(width/2)-100, y=92)
email_enter = Entry(reset_frame, textvariable=email_id_var, bd=2, width=30, justify=CENTER)
email_enter.place(x=(width/2)-100, y=132)
new_pas_enter = Entry(reset_frame, textvariable=password_var, bd=2, width=30, show='*', justify=CENTER)
new_pas_enter.place(x=(width)/2-100, y=172)
